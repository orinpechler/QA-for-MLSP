...
# This program implements the fourth QUBO presented, which uses unbalanced penalization. The assumptions under which the Multi-
...
# This for loop already calculates the penalty coefficient for lambda 1 and 2
penalty_value = 0
...
# Now, the QUBO is created that uses unbalanced penalization, for which 4 different penalty coefficients are initiated
lambda1 = Placeholder('lambda1')
lambda2 = Placeholder('lambda2')
lambda8 = Placeholder('lambda8')
lambda9 = Placeholder('lambda9')
...
# The penalty functions using unbalanced penalization use h(x) as specified in the paper, i.e. h(x) is equal to the club capacity
# minus the matches scheduled at a club in a certain round

# Penalty 8: First penalty function that represents the unbalanced penalization, with minus h(x) (the minus is implemented below)
penalty8 = sum(sum((club_capacities[c] - sum(sum(xVar[t][h]*U[h][r] for h in range(league_size)) for t in clubs[c])) for r in range(num_rounds)) for c in range(num_clubs))

# Penalty 9: Second penalty function that represents the unbalanced penalization, where h(x) is squared
penalty9 = sum(sum(((club_capacities[c] - sum(sum(xVar[t][h]*U[h][r] for h in range(league_size)) for t in clubs[c])) ** 2) for r in range(num_rounds)) for c in range(num_clubs))

# Together the penalties form the QUBO with the corresponding penalty coefficients
# Note that we take minus penalty 8, as is done for unbalanced penalization
H = lambda1*penalty1 + lambda2*penalty2 - lambda8*penalty8 + lambda9*penalty9
model4 = H.compile()
...
C = 1
D = 1
qubo, offset = model4.to_qubo(feed_dict = {"lambda1" : A, "lambda2" : B, "lambda8" : C, "lambda9" : D})
...
if False:
    ...
    #os.chdir()   # change working direction for output
    fileOUT = "QA-QUBO4-Sol-" + filename

if False:
    sampler = LeapHybridSampler()
    response = sampler.sample_qubo(qubo)
    #os.chdir()   # change working direction for output
    fileOUT = "HS-QUBO4-Sol-" + filename
...
pen8 = sum(sum((club_capacities[c] - sum(sum(solution_sample[f'xVar[{t}][{h}]']*U[h][r] for h in range(league_size)) for t in clubs[c])) for r in range(num_rounds)) for c in range(num_clubs))
pen9 = sum(sum(((club_capacities[c] - sum(sum(solution_sample[f'xVar[{t}][{h}]']*U[h][r] for h in range(league_size)) for t in clubs[c])) ** 2) for r in range(num_rounds)) for c in range(num_clubs))
total_energy = A*pen1 + B*pen2 - C*pen8 + D*pen9
print(f"Penalty1: {pen1}, Penalty2: {pen2}, Penalty8: {pen8}, Penalty9: {pen9}, Total Energy: {total_energy}")
...
with open(fileOUT, 'w') as f:
    ...
    f.write("\n")
    f.write(f"Penalty 1: {pen1}, Penalty 2: {pen2}, Penalty 8: {pen8}, Penalty 9: {pen9}, Total Energy: {total_energy}")
    f.write("\n")
    f.write("\n")
