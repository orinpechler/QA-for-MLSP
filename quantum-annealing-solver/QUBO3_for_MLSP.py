...
# This program implements the third QUBO presented, which uses binary encoding. The assumptions under which the Multi-
...
# This for loop already calculates the penalty value for lambda 1 and 2
penalty_value = 0
...
# Now, the QUBO is created that uses binary encoding, for which 3 different penalty coefficients are initiated
lambda1 = Placeholder('lambda1')
lambda2 = Placeholder('lambda2')
lambda7 = Placeholder('lambda7')

# Calculate the value M as specified in the paper for binary encoding
M = []
for c in range(num_clubs):  
    M_c = math.ceil(math.log2(club_capacities[c] + 1))
    M.append(M_c)
...
# Create the y auxiliary variable as specified in the thesis (that is, for each club and round, for values from 0 up to (M-1))
# Note that the paper uses notation of values from 1 to M. 
# As the capcities of the club differ and therefore value M, first the matrix of y is defined
yVar = []
for c in range(num_clubs):
    club_matrix = []
...
# Penalty 7: This penalty function uses the auxiliary variable from binary encoding to offset negative values for the difference in 
...
#            are penalized.
penalty7 = sum(sum(((sum((2 ** i) * yVar[c][r][i] for i in range(M[c])) + ((sum(sum(xVar[t][h]*U[h][r] for h in range(league_size)) for t in clubs[c]) - club_capacities[c]))) ** 2) for r in range(num_rounds)) for c in range(num_clubs))

# Together the penalties form the QUBO with the corresponding penalty coefficients
H = lambda1*penalty1 + lambda2*penalty2 + penalty7*lambda7
model3 = H.compile()
...
B = penalty_value
C = 1
qubo, offset = model3.to_qubo(feed_dict = {"lambda1" : A, "lambda2" : B, "lambda7" : C})
...
if True:
    ...
    #os.chdir()   # change working direction for output
    fileOUT = "QA-QUBO3-Sol-" + filename

if False:
    sampler = LeapHybridSampler()
    response = sampler.sample_qubo(qubo)
    #os.chdir()   # change working direction for output
    fileOUT = "HS-QUBO3-Sol-" + filename
...
pen7 = sum(sum(((sum((2 ** i) * solution_sample[f'yVar[{c}][{r}][{i}]'] for i in range(M[c])) + ((sum(sum(solution_sample[f'xVar[{t}][{h}]']*U[h][r] for h in range(league_size)) for t in clubs[c]) - club_capacities[c]))) ** 2) for r in range(num_rounds)) for c in range(num_clubs))
total_energy = A * pen1 + B * pen2 + C * pen7
print(f"Penalty1: {pen1}, Penalty2: {pen2}, Penalty7: {pen7}, Total Energy: {total_energy}")
...
with open(fileOUT, 'w') as f:
    ...
    f.write("\n")
    f.write(f"Penalty 1: {pen1}, Penalty 2: {pen2}, Penalty 7: {pen7}, Total Energy: {total_energy}")
    f.write("\n")
    f.write("\n")
