...
# This program implements the second QUBO presented, which uses domain-wall encoding. The assumptions under which the Multi-
...
# This for loop already calculates the penalty value for lambda 1, 2 and 6
penalty_value = 0
...
# Now, the QUBO is created that uses domain-wall encoding, for which 4 different penalty coefficients are initiated
lambda1 = Placeholder('lambda1')
lambda2 = Placeholder('lambda2')
lambda5 = Placeholder('lambda5')
lambda6 = Placeholder('lambda6')
...
# Create the y auxiliary variable as specified in the thesis (that is, for each club and round, for values from 0 up to the (capacity of the club - 1))
# (Note that in the paper different notation of 1 to capacity of club is used)
# As the capcities of the club differ, first the matrix of y is defined
yVar = []
for c in range(num_clubs):
    club_matrix = []
    for r in range(num_rounds):
        round_array = []
        for v in range(club_capacities[c]):
            round_array.append(None)
        club_matrix.append(round_array)
    yVar.append(club_matrix)

# Now in this matrix, each entry is defined as a binary variable
for c in range(num_clubs):
    for r in range(num_rounds):
        for v in range(club_capacities[c]):
            yVar[c][r][v] = Binary(f'yVar[{c}][{r}][{v}]')
...
# Penalty 5: This penalty function uses the auxiliary variable from domain-wall encoding to offset negative values for the difference in 
...
#            are penalized.
penalty5 = sum(sum(((sum(yVar[c][r][i] for i in range(club_capacities[c])) + ((sum(sum(xVar[t][h]*U[h][r] for h in range(league_size)) for t in clubs[c]) - club_capacities[c]))) ** 2) for r in range(num_rounds)) for c in range(num_clubs))

# Penalty 6: This penalty function enforces the domain-wall encoding
penalty6 = sum(sum(sum(yVar[c][r][i+1] - yVar[c][r][i+1]*yVar[c][r][i] for i in range(club_capacities[c] - 1)) for r in range(num_rounds)) for c in range(num_clubs))

# Together the penalties form the QUBO with the corresponding penalty coefficients
H = lambda1*penalty1 + lambda2*penalty2 + lambda5*penalty5 + lambda6*penalty6
model2 = H.compile()
...
C = 1
D = penalty_value
qubo, offset = model2.to_qubo(feed_dict = {"lambda1" : A, "lambda2" : B, "lambda5" : C, "lambda6" : D})
...
if True:
    ...
    #os.chdir()   # change working direction for output
    fileOUT = "QA-QUBO2-Sol-" + filename

if False:
    sampler = LeapHybridSampler()
    response = sampler.sample_qubo(qubo)
    #os.chdir()   # change working direction for output
    fileOUT = "HS-QUBO2-Sol-" + filename
...
# Manually, check penalty values that follow from the QUBO and the solution for it
...
pen5 = sum(sum(((sum(solution_sample[f'yVar[{c}][{r}][{i}]'] for i in range(club_capacities[c])) + ((sum(sum(solution_sample[f'xVar[{t}][{h}]']*U[h][r] for h in range(league_size)) for t in clubs[c]) - club_capacities[c]))) ** 2) for r in range(num_rounds)) for c in range(num_clubs))
pen6 = sum(sum(sum(solution_sample[f'yVar[{c}][{r}][{i+1}]'] - solution_sample[f'yVar[{c}][{r}][{i+1}]']*solution_sample[f'yVar[{c}][{r}][{i}]'] for i in range(club_capacities[c] - 1)) for r in range(num_rounds)) for c in range(num_clubs))
total_energy = A*pen1 + B*pen2 + C*pen5 + D*pen6
print(f"Penalty1: {pen1}, Penalty2: {pen2}, Penalty5: {pen5}, Penalty6: {pen6}, Total Energy: {total_energy}")
...
with open(fileOUT, 'w') as f:
    ...
    f.write("\n")
    f.write(f"Penalty 1: {pen1}, Penalty 2: {pen2}, Penalty 5: {pen5}, Penalty 6: {pen6}, Total Energy: {total_energy}")
    f.write("\n")
    f.write("\n")
